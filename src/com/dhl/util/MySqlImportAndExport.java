...
//    	}
     ...
     * @throws IOException 
     */  
    public static void export(String exportPath) throws IOException {  
    	InputStream is = MySqlImportAndExport.class.getClassLoader().getResourceAsStream("jdbc.properties");  
        Properties properties = new Properties();  
        properties.load(is);  
...
//        String exportPath = properties.getProperty("jdbc.exportPath");//导出路径  
        ...
        //注意哪些地方要空格，哪些不要空格  
        command.append("C:\\Program Files (x86)\\MySQL\\MySQL Server 5.0\\bin\\mysqldump -u").append(username).append(" -p").append(password)//密码是用的小p，而端口是用的大P。  
